...
* TODO Open Questions
  ...
  - [X] Should protected resources of EIDA-RS implement both HTTP POST and
    ...
    The question somehow messes up
    [[https://tools.ietf.org/html/rfc6750][RFC6750]]'s description of
    authenticated requests to protected resources of EIDA-RS. According to
    [[https://tools.ietf.org/html/rfc6750][RFC6750]] (section 2.1) protected
    resources within EIDA MUST implement authentication for HTTP GET requests
    using the Authorization request header field. Besides, protected
    resources MUST implement authenticated access by means of the HTTP POST
    method [[https://tools.ietf.org/html/rfc6750#section-2.2][RFC6750 section
    2.2]].
    Authenticated access using the HTTP GET method including the access token
    as a URI query parameter MUST NOT be implemented.
